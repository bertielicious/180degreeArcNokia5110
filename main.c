...
void main(void) 
{
    uint8_t angle, radius, ycoord, xcoord;
    uint16_t  opposite, adjacent;
    pageToRow ypixel;
   ...
   {
       for(radius = 0; radius <40; radius = radius + 1)
       {          
           // opposite = scaledSecondCheeseSine[angle] * radius;
           // adjacent = scaledSecondCheeseCosine[angle] * radius;
            opposite = sineLookUp[angle] * radius;
            adjacent = cosineLookUp[angle] * radius;
            ycoord = (opposite>>8);
            xcoord = 42 - (adjacent>>8);
